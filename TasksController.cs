...
namespace ToDoBackend.Controllers
{
    ...
    public class TasksController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetUserTasks()
        {
            ...
            .Where(t => 
                (t.IsIndividual && t.CreatedBy == userId) // bireysel görevse, sadece kendi görevlerin
                || 
                (t.CommunityId != null && userCommunities.Contains(t.CommunityId.Value)) // topluluk görevleriyse, topluluk üyesiysen
            )
            .ToListAsync();
        ...
        public async Task<IActionResult> UpdateTask(int taskId, [FromBody] TaskUpdateRequest updatedTask)
        {
            ...
            // Eğer görev bireyselse, görevi oluşturan kullanıcı mı diye kontrol et
            if (task.IsIndividual)
            {
                if (task.CreatedBy != userId)
                {
                    return Forbid("You are not authorized to view this personal task.");
                }
            }
